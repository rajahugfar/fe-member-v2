...
const AdminPoyDetail: React.FC = () => {
  ...
  const loadPoyDetail = async () => {
    try {
      setLoading(true);
      const response = await adminAPIClient.get(`/poy/${poyId}`);
      if (response.data.status === 'success' && response.data.data) {
        setPoy(response.data.data);

        // Load member info
        if (response.data.data.memberId) {
          loadMemberInfo(response.data.data.memberId);
        }
      }
    } catch (error: any) {
  ...
  const handleCancelPoy = async () => {
    ...
    try {
      setCancelling(true);
      await adminAPIClient.post(`/lottery/poy/${poyId}/cancel`, { reason: cancelReason });
