...
export const adminNotificationAPI = {
  ...
  getNotifications: async (params?: NotificationParams): Promise<Notification[]> => {
    const response = await adminClient.get('/admin/notifications', { params })
    return response.data.data
  },

  // Get unread notifications
  getUnreadNotifications: async (limit = 20): Promise<Notification[]> => {
    const response = await adminClient.get('/admin/notifications/unread', {
      params: { limit },
    })
    return response.data.data
  },
  ...
  getUnreadCount: async (): Promise<number> => {
    const response = await adminClient.get('/admin/notifications/unread/count')
