...
export default function MemberGameHistoryModal({
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-admin-card rounded-lg max-w-6xl w-full max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-admin-border sticky top-0 bg-admin-card z-10">
          <div>
            <h2 className="text-xl font-bold text-brown-100">ประวัติเกม</h2>
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg font-medium transition-colors ${
                  filter === 'all'
                    ...
                    : 'bg-admin-card text-brown-200 border border-admin-border hover:bg-admin-bg'
                }`}
              >
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-brown-400">ไม่พบประวัติเกม</p>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-admin-bg border-b border-admin-border">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-brown-200 uppercase">
                      วันที่/เวลา
                    </th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-brown-200 uppercase">
                      เกม
                    ...
                    <th className="px-4 py-3 text-right text-xs font-semibold text-brown-200 uppercase">
                      เดิมพัน
                    </th>
                    <th className="px-4 py-3 text-right text-xs font-semibold text-brown-200 uppercase">
                      ชนะ
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredHistory.map((game) => (
                    <tr key={game.betId} className="hover:bg-admin-bg">
                      <td className="px-4 py-3 text-sm text-brown-100">
                        {formatDate(game.betTime)}
                      </td>
                      <td className="px-4 py-3 text-sm text-brown-100">{game.gameName || game.gameCode}</td>
                      <td className="px-4 py-3 text-sm text-brown-300">{game.gameCode}</td>
                      <td className="px-4 py-3 text-sm text-right text-brown-100">
        ...
        {!loading && filteredHistory.length > 0 && (
          <div className="px-6 py-4 border-t border-admin-border bg-admin-bg">
            <div className="grid grid-cols-4 gap-4">
              ...
              <div className="text-center">
                <div className="text-sm text-brown-300">จำนวนเกม</div>
        ...
        <div className="flex justify-end p-6 border-t border-admin-border">
          <button
            onClick={onClose}
            className="px-6 py-2 bg-gray-200 text-brown-200 rounded-lg hover:bg-gray-300 transition-colors"
          >
            ปิด
          </button>
        </div>
