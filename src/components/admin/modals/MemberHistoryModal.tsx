...
export default function MemberHistoryModal({ isOpen, member, onClose }: MemberHistoryModalProps) {
  ...
  const getStatusBadge = (status: string) => {
    const badges: Record<string, string> = {
      ...
      cancelled: 'bg-admin-hover text-brown-100 border-admin-border',
    }
    ...
    return (
      <span className={`px-2 py-1 text-xs font-semibold rounded border ${badges[status] || 'bg-admin-hover text-brown-100'}`}>
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-admin-card rounded-lg max-w-5xl w-full max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-admin-border sticky top-0 bg-admin-card z-10">
          <div>
            <h2 className="text-xl font-bold text-brown-100">ประวัติการเติมเงิน/ถอนเงิน</h2>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredTransactions.map((tx) => {
                    ...
                    return (
                      <tr key={tx.id} className="hover:bg-admin-bg">
          ...
          <div className="px-6 py-4 border-t border-admin-border bg-admin-bg">
            <div className="grid grid-cols-3 gap-4">
              <div className="text-center">
                <div className="text-sm text-brown-300">ฝากเงินทั้งหมด</div>
              ...
              <div className="text-center">
                <div className="text-sm text-brown-300">ถอนเงินทั้งหมด</div>
              ...
              <div className="text-center">
                <div className="text-sm text-brown-300">ยอดรวม</div>
