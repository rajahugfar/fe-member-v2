...
export default function SuspendMemberModal({ isOpen, member, onClose, onSuccess }: SuspendMemberModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-admin-card rounded-lg max-w-md w-full">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-admin-border">
          <div>
            <h2 className="text-xl font-bold text-brown-100 flex items-center gap-2">
          ...
          <button
            onClick={handleClose}
            disabled={loading}
            className="p-1 hover:bg-admin-hover rounded transition-colors disabled:opacity-50"
          >
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="mb-4 p-4 bg-admin-bg border border-admin-border rounded-lg">
            <div className="text-sm text-brown-300 mb-1">สถานะปัจจุบัน</div>
            <div className="flex items-center gap-2">
              ...
              {member.status === 'inactive' && (
                <span className="px-3 py-1 bg-admin-hover text-brown-100 border border-admin-border rounded-full text-sm font-semibold">
                  ไม่ใช้งาน
                </span>
              )}
          ...
          {/* New Status */}
          <div className="mb-4">
            <label htmlFor="status" className="block text-sm font-semibold text-brown-200 mb-2">
            ...
            <select
              ...
              className="w-full px-4 py-2 border border-admin-border rounded-lg focus:ring-2 focus:ring-gold-500 focus:border-transparent"
              required
            >
          ...
          {selectedStatus === 'banned' && (
            <div className="mb-4">
              <label htmlFor="reason" className="block text-sm font-semibold text-brown-200 mb-2">
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-admin-border">
            <button
              ...
              onClick={handleClose}
              disabled={loading}
              className="px-6 py-2 border border-admin-border text-brown-200 rounded-lg hover:bg-admin-bg transition-colors disabled:opacity-50"
            >
              ยกเลิก
            </button>
            <button
